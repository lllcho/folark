...
from app.plugins.builtin_plugin import (
    audio_plugin,
    docx_plugin,
    epub_plugin,
    image_plugin,
...
class BuiltinPlugin(BasePlugin):
    ...
    @task_handler(
        handler_name="pdf_extract",
        handler_type=TaskHandlerType.EXTRACT,
        source_types=["pdf"],
        handler_mode=TaskHandlerMode.INSTANT,
        description="提PDF文本内容",
    )
    def pdf_extract(self, ctx: PluginContext) -> ExtractResult | None:
        return pdf_plugin.extract(ctx)

    @task_handler(
        handler_name="pdf_thumbnail",
        handler_type=TaskHandlerType.THUMBNAIL,
        source_types=["pdf"],
        handler_mode=TaskHandlerMode.INSTANT,
        description="生成PDF首页缩略图",
    )
    def pdf_thumbnail(self, ctx: PluginContext) -> ThumbnailResult | None:
        return pdf_plugin.thumbnail(ctx)

    @task_handler(
        handler_name="pdf_preview",
        handler_type=TaskHandlerType.PREVIEW,
        source_types=["pdf"],
        handler_mode=TaskHandlerMode.ON_DEMAND,
        description="PDF预览",
    )
    ...
    @task_handler(
        handler_name="txt_convert_epub",
        handler_type=TaskHandlerType.CONVERT,
        source_types=["txt"],
        target_types=["epub"],
        handler_mode=TaskHandlerMode.ON_DEMAND,
        description="TXT转EPUB格式",
    )
    ...
    @task_handler(
        handler_name="docx_preview",
        handler_type=TaskHandlerType.PREVIEW,
        source_types=["docx"],
        handler_mode=TaskHandlerMode.ON_DEMAND,
        description="DOCX文件在线预览",
    )
    ...
    @task_handler(
        handler_name="xlsx_extract",
        handler_type=TaskHandlerType.EXTRACT,
        source_types=["xlsx"],
        handler_mode=TaskHandlerMode.INSTANT,
        description="提取Excel表格内容",
    )
    def xlsx_extract(self, ctx: PluginContext) -> ExtractResult | None:
        return xlsx_plugin.extract(ctx)

    @task_handler(
        handler_name="xlsx_preview",
        handler_type=TaskHandlerType.PREVIEW,
        source_types=["xlsx"],
        handler_mode=TaskHandlerMode.ON_DEMAND,
        description="Excel在线预览",
    )
